...
def getBookingJiniInventory(email, password, property_code):
    ...
    def fetch_room_types(auth_token, property_code):
        ...
        if response.status_code == 200:
            ...
            print(f"Failed to fetch room types. Status code: {response.status_code}")
            return []

    def fetch_rates(auth_token, property_code, room_id, start_date, end_date):
        rate_url = f"https://cm.bookingjini.com/extranetv4/rates/get-rates/{property_code}/{room_id}/{start_date}/{end_date}"
        rate_headers = {'Authorization': f'Bearer {auth_token}'}

        rate_response = requests.get(rate_url, headers=rate_headers)
        if rate_response.status_code == 200:
            result = rate_response.json()

            with open(f"{property_code}_{room_id}_Rates.json", "w") as json_file:
            ...
            return roomPriceInfoList
        else:
            print(f"Failed to fetch rates with status code: {rate_response.status_code}")
            return []
    ...
    for room in room_types:
        ...
        for arrival_date, max_rooms in max_inventory.items():
            room_rates = fetch_rates(auth_token, property_code, room_id, arrival_date, arrival_date)
            
            room_info["inventory"].append({
                "arrivalDate": arrival_date,
                "totalRooms": total_rooms,
                "availableRooms": max_rooms,
                "roomPrice": room_rates[0]["roomPrice"] if room_rates else None
            })
