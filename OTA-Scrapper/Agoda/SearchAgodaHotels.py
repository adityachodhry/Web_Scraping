...
def get_hotels_for_city(city_id, checkin=None, checkout=None):
    if not checkin:
...
def extract_hotel_info(response_json):
    ...
    if 'data' in response_json:
        properties = response_json['data']['citySearch']['properties']

        totalProperties = response_json['data']['citySearch']['searchResult']['searchInfo']['totalFilteredHotels']

        for property in properties :
            informationSummary = property['content']['informationSummary']
            image_urls = []
            images = property['content']['images']['hotelImages']
            reviews = property['content']['reviews']
            for img in images :
                urls = img['urls']
                for url in urls:
                    if 'agoda' in url['value'] :
                        image_urls.append(f"https:{url['value'].split('?')[0]}")
            try :
            ...
            try :
                hotel_info = {
                    ...
                    'Name' : informationSummary['displayName'],
                    'Rating' : informationSummary.get('rating','N/A'),
                    'Location' : f"{informationSummary['address']['area']['name']}, {informationSummary['address']['city']['name']}",
                    'Price' : price,
                    'ImageUrls' : image_urls,
                    'DetailLink' : f"https://www.agoda.com/en-in{informationSummary['propertyLinks']['propertyPage']}",
                    'ReviewSummary' : {
                        'RatingScore' : reviews['cumulative']['score'],
                        'ReviewCount' : reviews['cumulative']['reviewCount']
                    }
                }
    ...
    return totalProperties, hotels_info