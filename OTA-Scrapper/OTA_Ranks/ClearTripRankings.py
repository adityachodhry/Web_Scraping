...
def fetch_cleartrip_hotel_rankings(city_name, state_name, country_code, cityId):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # with open('cleartrip_Row.json', 'w') as file:
            ...
            for slot in slots_data:
                if slot['slotData']['type'] == 'HOTEL_CARD_LIST':
                    ...
                    for card in hotelCards:
                        ...
                        ranks['Name'] = eventData['h_hotel_name']
                        ranks['Rating'] = eventData['h_star_category']
                        ranks['location'] = eventData['h_search_destination_city']
                        # ranks['city'] = eventData['h_search_destination_category']
                        ranks['ReviewSummary'] = {
                            "RatingScore": eventData['h_user_rating'],
                            # "RatingCount": eventData['h_total_amount'],
                        }

                        # response.slotsData[1].slotData.data.hotelCardList[0].data.locality
